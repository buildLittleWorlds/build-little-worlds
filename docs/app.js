...
const SITE_TITLE = "Philosophy of Deployment";
const MAX_HOME_POSTS = null; // show all published posts on the home index
...
async function renderPostList(container, limit = null) {
  try {
    ...
    if (count) {
      ...
      count.textContent = `${String(n).padStart(2, "0")} ${n === 1 ? "essay" : "essays"} · published`;
    }
  } catch (error) {
...
function renderPostCard(post) {
  ...
  const cat = post.category ? `<span class="cat">${escapeHtml(post.category)}</span> · ` : "";
  meta.innerHTML = `${cat}${escapeHtml(formatDate(post.date))}`;
...
function renderMissingPost(posts) {
  ...
  postBody.innerHTML = `
    <header class="post-header">
      <p class="post-meta">Not found</p>
      <h1>Post not found</h1>
      <p class="post-description">The requested slug does not match the current analysis post.</p>
    </header>
    <div class="markdown-body">
      <p>Choose the current analysis instead:</p>
      <ul>${links}</ul>
    </div>
  `;
}

// Reading progress — fills as the reader scrolls through the article.
